      ...
      <div className="mx-auto max-w-7xl">
        ...
        <div className="grid gap-10 lg:grid-cols-[0.9fr_1.1fr] lg:items-end">
          <div>
            ...
            <h1 className="font-display text-5xl leading-[1.05] tracking-tight text-[#3A2B29] md:text-6xl">
        ...
        >
          ...
          {filteredWork.length === 0 && (
            <div className="col-span-full flex min-h-[280px] flex-col items-center justify-center rounded-3xl border-2 border-dashed border-[#E8D5D4] bg-white/60 p-12 text-center text-[#A67571]">
    ...
    >
      ...
      <div className="flex min-h-[430px] flex-col p-6">
        ...
        {(item.organization || item.location) && (
          <p className="mt-1 text-sm font-light italic text-[#8C7A78]">
            {[item.organization, item.location].filter(Boolean).join(" · ")}
          </p>
        )}

        <div className="mt-6 space-y-4">
          <div className="border-l-2 border-[#F2EAE9] pl-4">
            <span className="mb-1 block text-[10px] font-bold uppercase tracking-wider text-[#A67571]">
              The Problem
            </span>
            <p className="text-sm leading-relaxed text-[#5C4D4B]">
              {item.problem}
            </p>
          </div>
          <div className="border-l-2 border-[#C28C88] pl-4">
            <span className="mb-1 block text-[10px] font-bold uppercase tracking-wider text-[#A67571]">
              What I Built
            </span>
            <p className="text-sm font-medium leading-relaxed text-[#3A2B29]">
              {item.contribution}
            </p>
          </div>
        </div>
        ...
        <div className="mt-auto flex flex-wrap gap-3 pt-8">
          ...
          {item.links.repoUrl ? (
            <a
              ...
              className="inline-flex items-center gap-2 rounded-full border border-[#E8D5D4] bg-[#FAF7F5] px-4 py-2 text-sm font-bold text-[#3A2B29] transition-colors hover:border-[#C28C88] hover:text-[#C28C88] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-rose-500"
              target="_blank"
              rel="noreferrer"
            >
            ...
            <span className="inline-flex items-center gap-2 rounded-full border border-[#E8D5D4] bg-[#FAF7F5] px-4 py-2 text-sm font-bold text-[#5C4D4B]">
