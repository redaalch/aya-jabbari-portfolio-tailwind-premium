...
export function About() {
  ...
  return (
    <section
      id="about"
      className="relative min-h-screen bg-[#FAF7F5] px-6 py-24 text-[#3A2B29] md:px-12 md:py-32 lg:px-20"
    >
      <div
        className="absolute left-0 top-0 h-px w-full bg-gradient-to-r from-transparent via-[#E8D5D4] to-transparent opacity-70"
        aria-hidden="true"
      />
          ...
          <div className="flex flex-col self-start lg:sticky lg:top-32 lg:col-span-5">
            ...
            <ul className="space-y-5 border-l border-[#E8D5D4] py-2 pl-6 text-sm text-[#5C4D4B]">
              <li className="group flex items-center gap-4">
                <MapPin
                  className="h-[18px] w-[18px] text-[#C28C88] transition-transform group-hover:-translate-y-0.5"
                  strokeWidth={1.5}
                  aria-hidden="true"
                />
              ...
              <div className="flex flex-col gap-10">
                {focusAreas.map((item, index) => (
                  <div key={item.label} className="group flex gap-6">
                    <span className="pt-1 font-display text-2xl text-[#E8D5D4] transition-colors group-hover:text-[#C28C88]">
                      0{index + 1}
                    </span>
                    <div>
                      <h3 className="mb-2 text-2xl font-semibold text-[#3A2B29]">
                        {item.label}
                      </h3>
                      <p className="font-light leading-relaxed text-[#8C7A78]">
                        {item.description}
                      </p>
                    </div>
                  </div>
              ...
              <div className="relative ml-2 space-y-12 border-l border-[#E8D5D4] pb-2">
                {education.map((item) => (
                  <div key={`${item.title}-${item.date}`} className="group relative pl-8">
                    ...
                    <h3 className="mb-2 font-display text-2xl leading-snug text-[#3A2B29]">
                      {item.title}
                    </h3>
                    <p className="text-sm font-light text-[#8C7A78] md:text-base">
                      {item.school}
                    </p>
                  </div>
                ))}
                ...
                {languages.map((item) => (
                  <div key={item.language} className="group">
                    <div className="relative mb-2 flex min-h-8 items-center">
                      <p className="font-display text-2xl text-[#3A2B29] transition-all duration-300 group-hover:-translate-y-2 group-hover:opacity-0">
                        {item.language}
                      </p>
                      <p className="absolute left-0 font-display text-2xl text-[#C28C88] opacity-0 transition-all duration-300 translate-y-2 group-hover:translate-y-0 group-hover:opacity-100">
...
function SectionTitle({ id, icon, children }: SectionTitleProps) {
  return (
    <h3
      id={id}
      className="mb-10 flex items-center gap-3 border-b border-[#E8D5D4] pb-4 text-xs font-bold uppercase tracking-[0.2em] text-[#A67571]"
    >
      {icon}
      {children}
    </h3>
